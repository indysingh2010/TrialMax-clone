...
//	01-29-2014	7.27		Added methods to handle gestures for windows 7
//							and onwards
//	02-21-2014	7.29		Added method to display toolbar on blank presentation
//==============================================================================
	...
	if(bResult)
	{
		switch(gi.dwID)
		{
			case GID_BEGIN:
				m_bMouseMode = FALSE;
				m_bGestureHandled = FALSE;
				m_gestureStartPoint = m_gestureCurrentPoint = gi.ptsLocation;
...
void CMainView::HandlePan(GESTUREINFO gi)
{
	...
	iDistX = p.x - m_gestureStartPoint.x;
	iDistY = p.y - m_gestureStartPoint.y;
	...
	m_ctrlTMView.DoGesturePan(p.x - m_gestureCurrentPoint.x, p.y - m_gestureCurrentPoint.y);

	// update current location
	m_gestureCurrentPoint = p;

	if (m_gestureCurrentPoint.x == m_gestureStartPoint.x &&
		m_gestureCurrentPoint.y == m_gestureStartPoint.y)
		return;

	DWORD currentTime = GetTickCount();
	long timeInterval = currentTime - m_gestureStartTime;


	// 3. Swipe right to left to advance to the next page and swipe left to right to go to the previous page

	// check for inertia
	// check if distnace between stating and ending point is greater than 1/20 of screen
	// check time interval to find if its pan or swipe
	if (abs(iDistX) > iMonitor_width/20 && gi.dwFlags == GF_INERTIA && timeInterval < 800) {
	...
	if (m_gestureStartPoint.y <= iMonitor_height/4
		&& iDistY < iMonitor_height/4) {
		DisplayKeyboardIconGesture();
		m_bGestureHandled = TRUE;
		return;
	}
	...
	if (m_gestureStartPoint.y >= (iMonitor_height - iMonitor_height/4)
		&& iDistY < iMonitor_height/4) {
		DisplayToolbarGesture();
		m_bGestureHandled = TRUE;
		return;
	}
}
...
void CMainView::HandleZoom(GESTUREINFO gi)
{
	...
	if (gi.dwFlags == GF_BEGIN) {
		...
		// limit horizontal pinch
		//if (abs(m_gestureStartPoint.x - gi.ptsLocation.x) >= 50)
		//	return;

		// center point to zoom
		// TODO: clear this function, tmview and tmlead if client is happy with this implementation
		POINTS p;
		p.x = (m_gestureCurrentPoint.x + gi.ptsLocation.x) / 2;
		p.y = (m_gestureCurrentPoint.y + gi.ptsLocation.y) / 2;

		// zoom factor
		FLOAT scale = (FLOAT)gi.ullArguments / (FLOAT)m_ullArguments;

		m_ctrlTMView.DoGestureZoom(scale, m_gestureCurrentPoint.x, m_gestureCurrentPoint.y, gi.ptsLocation.x, gi.ptsLocation.y);

		// update current point and distance
		m_gestureCurrentPoint = gi.ptsLocation;
		m_ullArguments = gi.ullArguments;
...
//
//==============================================================================
void CMainView::DisplayKeyboardIconGesture()
{
	if (m_pVKBDlg) {
		// if y is increasing, its swipe down
		if (m_gestureCurrentPoint.y > m_gestureStartPoint.y) {
			if (!m_pVKBDlg->IsWindowVisible())
				m_pVKBDlg->ShowWindow(SW_NORMAL);
		}
		else if (m_gestureCurrentPoint.y < m_gestureStartPoint.y) {
			if (m_pVKBDlg->IsWindowVisible())
...
//
//==============================================================================
void CMainView::DisplayToolbarGesture()
{
	if (m_pToolbar) {
		// if y is increasing, its swipe down
		if (m_gestureCurrentPoint.y > m_gestureStartPoint.y) {
